import { Controller, Get, Res, HttpStatus, Param, NotFoundException, Post, Body,
Query, Put, Delete } from '@nestjs/common';
...
export class BlogController {
    ...
    async addPost(@Res() res, @Body() createPostDTO: CreatePostDTO) {
        ...
        return res.status(HttpStatus.OK).json({
            message: 'Пост был успешно размещен',
            post: newPost
        })
    }
    ...
    async editPost(@Res() res, @Query('postID', new ValidateObjectId()) postID, @Body() createPostDTO: CreatePostDTO) {
        ...
        return res.status(HttpStatus.OK).json({
            message: 'Пост отредактирован успешно',
            post: editedPost
        })
    }
